...
【信息缺失处理规则 — 硬规则，必须遵循】
如果目标公司档案中某字段为"未知"或空值，该维度的子分严格取以下值，禁止取其他值：
  - 规模/融资为"未知" → size_score 必须选 10，不得选 0 或 5
  - 近期动态为空/非采购信号的品牌描述 → timing_score 必须选 10，不得选 0 或 5
  - 行业为"未知" → 选 12（边缘行业）
  - evidence 字段填写"档案中该项为未知，取中性评分"

行业匹配规则：
行业评分只看行业大类别是否一致，不看竞争关系。
即使目标公司与你的产品存在功能重叠或直接竞品，只要行业分类一致，仍按行业匹配度正常给分。
竞品关系不是扣分理由。
